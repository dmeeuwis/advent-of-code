...
(defn find-finished-job [time busy-worker]
  (let [job-time (inc (- (int (.charAt (busy-worker :step) 0)) 65))]
    (if (>= time (+ (busy-worker :start) job-time))
      busy-worker
      nil)))
...
(defn order-steps [all-claims all-steps workers]
  (loop [claims all-claims, steps all-steps, work-in-progress [], order "", time 0]
    ...
    (if (and (empty? steps) (empty? work-in-progress))
      ...
      (let [finished-workers (filter #(find-finished-job time %) work-in-progress)
        ...
        (recur (filter #(not (in just-completed-steps (first %))) claims) 
...
(let [all-claims (-> (first *command-line-args*)
  ...
  (println (order-steps all-claims 
